...
for period in periods:
    ...
    for d in range(1,dd):
        try:
            ...
            # DB SCAN
            vect = CountVectorizer(stop_words = s_words,ngram_range=(1, 1),min_df=1)
            X = vect.fit_transform(corpus)

            feature = pd.DataFrame(X.toarray())
            model = DBSCAN(eps=0.5,metric="cosine",min_samples=2)
            ...
            fn = str(now) + "_" + str(period) + "_" + "ME_" +  "DBSCAN_TR_Summary" + ".txt"
            s_df.to_csv(path + fn,header=None)
            
        except:
            s_df = pd.DataFrame(['','']).T
            s_df.columns=['Sentences','cnt']
            fn = str(now) + "_" + str(period) + "_" + "ME_" +  "DBSCAN_TR_Summary" + ".txt"
            s_df.to_csv(path + fn,header=None)
